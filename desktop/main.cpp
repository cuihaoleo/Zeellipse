...
void dynamic_erode(Mat &bin, Point center, double dt=M_PI/180) {
    _dynamic_erode_dilate(bin, center, dt, 0, 255);
}

void dynamic_dilate(Mat &bin, Point center, double dt=M_PI/180) {
...
RotatedRect get_rbox(Mat &im) {
    ...
    Mat vis;
    cvtColor(im, vis, CV_GRAY2BGR);
    for (auto p: ellipses) {
        auto r = p.second;
        ellipse(vis, r, Scalar(0, 0, 255), 2);
    }
    imwrite("oooo.jpg", vis);
...
int main(int argc, char** argv)
{
    ...
    double ratio = 1.0 * box.size.width / box.size.height;
    cout << eccentricity(box) << endl;
    for (int i=0, j=0; j<seq2.size(); i=j) {
        while (j<seq2.size() and seq2[j]+1 < 0 and seq[j]>avg)
            j++;
        if (j>i) {
            double r = (i+j)/2.0;
            RotatedRect el(box.center, Size(2*r, 2*r/ratio), box.angle);
            ellipse(blur, el, Scalar(255));
            //cout << (i+j)/2 << ' ';
            i = j;
        }
        while (j<seq2.size() and not(seq2[j]+1<0 and seq[j]>avg))
            j++;
    }
    /*
    for (double i: seq)
